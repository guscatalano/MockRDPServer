...
public class DesktopInputTests
{
    ...
    [Fact]
    public void DvcConsole_TypesMessageAndQueuesSendOnEnter()
    {
        using var d = new FakeDesktop(1024, 768);
        d.OpenApp("DVC Console");
        Assert.Contains("DVC Console", d.FocusedTitle);

        Type(d, "hi");                       // message field is focused by default
        Assert.Empty(d.TakeDvcSends());      // nothing queued until Enter
        d.OnInput(Key(0x1C));                // Enter

        var sends = d.TakeDvcSends();
        Assert.Single(sends);
        Assert.Equal("APP", sends[0].Channel);
        Assert.Equal("hi", sends[0].Text);
        Assert.Empty(d.TakeDvcSends());      // read-and-clear
    }
